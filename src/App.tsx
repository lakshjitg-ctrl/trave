...
export default function App() {
  ...
  const finalizeBookingWithPayment = (paidOnline) => {
  ...
  const verifyPaymentAndBook = () => {
    if (!paymentScreenshot) {
      setVerificationError("Please upload a payment screenshot first.");
      return;
    }
  ...
  const askAI = () => {
    ...
    setTimeout(() => {
      setAiResponse(
        `AI Travel Assistant Recommendation:\nFor your route, trains run frequently. Ensure you carry a valid photo ID during your journey.`
      );
      ...
      <div>
        ...
        <div className="grid grid-cols-4 gap-4">
          {[
            ...
            { label: "Train", color: "bg-indigo-100 text-indigo-600" },
            { label: "Bus", color: "bg-emerald-100 text-emerald-600" },
            { label: "Hotel", color: "bg-orange-100 text-orange-600" },
          ].map((item, i) => (
            <button
              key={i}
          ...
          {bookings.slice(0, 3).map((booking) => (
            ...
            >
              <div className="flex items-center space-x-4">
                <div className="p-2 rounded-full bg-indigo-100 text-indigo-600 font-bold text-xs">
  ...
  const renderBookTicket = () => (
    <div className="animate-fade-in pb-24">
      ...
      >
        ...
        <button
          type="submit"
          className="w-full py-4 bg-blue-600 hover:bg-blue-700 text-white font-semibold rounded-2xl shadow-lg shadow-blue-500/25 transition-all"
        >
      ...
      <div className="animate-fade-in pb-24">
        ...
        <div className="space-y-4 mb-6">
          <div className="relative">
            <input
              ...
              className="w-full px-4 py-3 bg-white border border-slate-200 rounded-xl focus:ring-2 focus:ring-blue-500 focus:outline-none"
            />
          </div>
        ...
        <div className="space-y-3">
          {filtered.map((booking) => (
            ...
            >
              <div className="flex items-center space-x-4">
                <div className="p-3 rounded-xl bg-indigo-100 text-indigo-600 font-bold">
                  {booking.transportType?.[0] || "T"}
                </div>
                <div>
    ...
    <div className="min-h-screen bg-slate-100 text-slate-900 font-sans select-none overflow-x-hidden">
      ...
      <header className="bg-white border-b border-slate-200 sticky top-0 z-40 px-4 py-3 flex justify-between items-center shadow-xs">
        ...
        <button
          ...
          className="px-3 py-1.5 text-xs bg-blue-50 text-blue-600 font-semibold rounded-full hover:bg-blue-100 transition-colors"
        >
          AI Assistant
        </button>
      </header>
            ...
            <div className="bg-[#1d61e0] text-white px-4 py-3 flex justify-between items-center shadow-md">
              <div className="flex items-center space-x-3">
                <button
                  ...
                  className="text-white hover:bg-blue-700 p-1 rounded-full transition-colors font-bold"
                >
                  &larr;
                </button>
              ...
              <button
                onClick={() => setSelectedTicket(null)}
                className="text-white hover:bg-blue-700 p-1 rounded-full transition-colors font-bold text-lg"
              >
                &times;
              </button>
            </div>
              ...
              <div className="rounded-xl shadow-md overflow-hidden relative border-0 bg-[#f0f4f8]">
                ...
                <div className="bg-[#f0f4f8] text-slate-900 p-4 space-y-3.5 text-xs relative">
                  ...
                  <div className="relative py-3 my-1">
                    <div className="border-b-4 border-dashed border-slate-400 w-full"></div>
                    <div className="absolute -left-6 -top-2.5 w-6 h-6 bg-[#e9eff5] rounded-full shadow-inner"></div>
                    <div className="absolute -right-6 -top-2.5 w-6 h-6 bg-[#e9eff5] rounded-full shadow-inner"></div>
                  </div>
